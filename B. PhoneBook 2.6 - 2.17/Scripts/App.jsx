...
const App = () => {
  ...
  const addName = (event) => {
    ...
    if (nameExists) {
      ...
      //update logic
      nameService.update(nameExists.id, nameObject).then((updatePerson)=>{
        setPersons((prevPerson)=>{
          prevPerson.id === nameExists.id ? updatePerson : persons
        })
        setFilteredPerson((prevFilteredPerson)=>{
          prevFilteredPerson.id === nameExists.id ? updatePerson : persons
        })
        setSuccessMessage(`${updatePerson.name} has been updated is successfully!`)
        setTimeout(()=>{
          setSuccessMessage("")
        }, 4000)
      })
      .catch((error)=>{
        ...
        setErrorMessage(`information: ${nameExists.name} has already been removed from the server!`)
        setTimeout(()=>{
          setErrorMessage("")
        }, 4000)
